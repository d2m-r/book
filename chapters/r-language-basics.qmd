...
Comparison operators are actually a subset of *logical* operators, which combine or modify logical values (`TRUE` or `FALSE`).
Aside from the comparison operators above, there are 3 other logical operators:
...
It helps to think of the `!` as "not": `!=` is "not equal to".
`!TRUE` is `FALSE`.
