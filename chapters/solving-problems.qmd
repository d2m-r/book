...
#### tl;dr -- Inevitable Error Type Families
...
-   `non-numeric argument to a binary operator`: at least one value/variable is not of a function's required class
    -   do you need to coerce variables or values (e.g., numeric to string; string to POSIX/date)
    -   check functions' documentation for argument class requirements
-   `replacement has/is [...]`: usually means objects are not the same length
    -   did you use any functions that combine scalars into a single scalar rather than a vector? e.g., using paste() vs c()/list()
    -   do all objects handle NA observations the same way? (e.g., one vector filtered them out and one didn't)
-   `object not found`: the object does not exist in your environment
    -   did you misspell or mis-capitalize?
    -   did you forget to run or source code in your current R session? 
-   `function not found`: the function is not loaded in the R session
    -   did you load the necessary package?
    -   did you misspell or mis-capitalize?
    -   do you have punctuation errors or unmatched parentheses?
-   `unexpected symbol`: punctuation is used incorrectly
    -   do you have unmatched or unnecessary parentheses?
    -   did you forget a comma?
    -   did you use quotes to try to refer to an existing object?
-   `unexpected numeric constant`:  same as above, and value after missing punctuation is a numeral
