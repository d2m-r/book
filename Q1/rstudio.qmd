...
RStudio looks like a grid of four panels which you can rearrange as you like. Your *source* and *console* panels will always be two of these four. The other two panels will be one or more tabs of useful information and actions. 
...
In both the default layout and the example layout above (@fig-example-rstudio-layout), the *source* pane is the top left quarter of the screen. Here it is outlined in orange.
...
3. You can run code directly from the source pane by selecting the code you want to run and pressing `Cmd + Enter`^[As a reminder, this book presumes use of Mac OS. Documentation for shortcuts and interface actions for Windows and Linux is plentiful elsewhere, including the [Posit website](https://docs.posit.co/ide/user/ide/reference/shortcuts.html)], or by clicking the "Run" button in the toolbar above the source pane.
...
If something goes wrong, or if there's just something R thinks you should know about the code you're running, you'll see an error, warning, or message as well. <!-- we cover these types of messages in the solving problems chapter -->
...
Pay attention to both where the output appears and when it appears (or really, when it does *not* appear). The comments (`# Which look like this`) are there for you-the-human, not R. 
...
Critically, the console does not save your code when your R session ends^[Your console input is *temporarily* saved in the history tab. More on that in @sec-history-pane.], so you should not use it to run any code that you anticipate needing to run more than once or that may require editing.
...
Can you install and load packages from the packages pane? Yes.
Should you? No. You should use the `install.packages()` and `library()` functions to keep your project files and code self-contained and reproducible.
...
Use it as a shortcut to skip the time consuming grunt work for *things you already know how to do yourself.*
Computers are, ultimately, pretty stupid. 
...
Run something you thought was temporary in the console but now you want to keep it? Find the command and add to source.
Need to rerun a quick, non-source-worthy command without scrolling up through hundreds of lines of console output? Find it in the history and run it again.
Cleaned up the console two seconds before realizing you need to run that command you just ran one more time? No worries. It's in the history.
...
When you try to use my code, you probably don't have the file in a "Natalie" folder on your computer^[Unless your name is also Natalie, I suppose.], so it won't work. 
You'll have to manually go in and change every file path.

Better practice is to use *relative* file paths. 
Relative to what? Relative to your working directory.
...
Now when you close my repo to your own computer, you can call it anything you want and save it anywhere you want, but as long as you keep the same relative file structure, my code will still work.
...
But also don't do that. Just use the `install.packages()` and `library()` functions in your source scripts.
...
Click the globe (I think that's what that is?) icon to open the package's website in your browser, if one is available.

#### Help pane

Speaking of...the **help pane** is where you'll find documentation for R functions, packages, and other resources.
...
You'll only see autocomplete selections for packages and functions that you have loaded, but you can type in anything you want and it will search for the term in CRAN documentation. (We'll talk more about CRAN in @sec-solving-problems).
...
The **plots pane** is (unsurprisingly) where you'll see plots and visualizations created in R.
...
RStudio is extremely customizable with options and keyboard shortcuts, and you can set it up to work the way you like. You can ignore most of the settings for now, but there are a few that you should be aware of from the start. 
...
You can access these settings through the "Tools" menu. When it comes to initial setup of RStudio, you'll want to focus on the global settings first. You can then adjust project settings as needed.
